...
import { GET_CHANNEL_ROUTE, GET_CONTACTS_DM } from "@/utils/constants";
...
const ContactContainer = () => {
  ...
  useEffect(() => {
    const getContacts = async () => {
      const response = await axios.get(GET_CONTACTS_DM, {
    ...
    const getChannels = async () => {
      const response = await axios.get(GET_CHANNEL_ROUTE, {
        withCredentials: true,
      });
...
const Logo = () => {
  return (
    <div className="flex p-5  justify-start items-center gap-2">
      ...
      <span className="text-3xl font-semibold ">Harmoniq</span>
    </div>
  );
};
